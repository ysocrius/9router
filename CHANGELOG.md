...
## Features
- Context Auto-Compaction: 9Router now automatically prunes older messages when a request's estimated token count approaches the provider's context limit. Prevents Cerebras/Groq 429 "tokens per minute" errors caused by oversized histories.
- Smart Drop Strategy: Tool call + tool result pairs are treated as atomic units and always dropped together. The most recent messages (configurable `minRecentMessages`, default 4) are always preserved.
- CJK-Aware Token Estimation: Estimator uses `chars/2` for text with >20% non-ASCII characters (CJK, emoji) instead of the standard `chars/4`, significantly improving accuracy for multilingual conversations.
- Per-Provider Limits: Context window and image token costs are configurable per provider in `open-sse/config/providerLimits.js` (Cerebras: 45k, Groq: 100k, Anthropic: 180k, OpenAI: 120k, etc.).
- Observability Headers: Compacted responses include `X-9Router-Compacted: true`, `X-9Router-Compacted-Dropped`, `X-9Router-Tokens-Before`, and `X-9Router-Tokens-After` headers for monitoring.

## Bug Fixes
- Hard rejection with OpenAI-compatible error (`context_length_exceeded`) when compaction cannot reduce the request to fit (e.g. system prompt alone exceeds budget).
- Effective budget correctly accounts for `max_tokens` reservation (defaults to 4096 when absent or zero).
