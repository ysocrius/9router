...
export class FreebuffExecutor extends BaseExecutor {
  ...
  async getSession(token, proxyOptions) {
    let session = sessionCache[token];
    const now = Date.now();
    
    // Check if session is cached and valid (expiry has at least 1 minute buffer)
    if (session && session.instanceId && session.expiresAt && (new Date(session.expiresAt).getTime() - now > 60000)) {
  ...
  async execute(options) {
    ...
    try {
      sessionInstanceId = await this.getSession(token, proxyOptions);
    } catch (error) {
      if (error.status === 503) {
    ...
    try {
      ...
      if (!result.response.ok) {
        ...
        if (errorText.includes("freebuff_update_required") || 
          ...
          log?.info?.("FREEBUFF", `Session invalid (${errorText.slice(0, 100)}), clearing cache and retrying`);
